...
#SBATCH --output=logs/mobilenet_base_%j.out
#SBATCH --error=logs/mobilenet_base_%j.err
#
# Experiment 2: pretrained baseline
#   - ImageNet-pretrained MobileNetV3 encoder
#   - dirty (original) LiDAR GT
#   - silog only
#   - no ASPP, no EAGLE, no VN
# Pair with scratch.sh to isolate the contribution of ImageNet pretraining.
...
PROJ_DIR="${PROJ_DIR:-approaches/Mobilenet/results/base}"
