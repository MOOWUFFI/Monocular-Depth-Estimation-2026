"""Paper-aligned EAGLE module for depth estimation (model side only).

Reference:
    Kim et al., "EAGLE: Eigen Aggregation Learning for Object-Centric
    Unsupervised Semantic Segmentation", CVPR 2024.

What this implements (faithful to the paper):

  1. Adjacency `A = A_color + A_seg` (paper §3.2.1 III — simple sum, not a
     convex combination).
  2. Symmetric normalized Laplacian `L_sym = I - D^{-1/2} A D^{-1/2}`.
  3. Eigendecomposition; keep the k smallest non-trivial eigenvectors V̂.
  4. Learnable cluster centers C ∈ R^{k × num_clusters}, trained jointly
     with the rest of the model. Soft cluster assignment is
     `P = V̂ C` followed by softmax — this is the differentiable mini-batch
     K-means in cosine space described in the paper.
  5. EiCue cluster-assignment map `M_eicue = argmax_c softmax(P)_c` (paper
     Eq. 2, exposed only for visualisation; the loss uses the soft form).

What this module does NOT do:

  * It does not modify the input feature map. The paper uses EiCue purely
    as a *supervisory signal* for an object-centric contrastive loss; it
    is not a feature-aggregation block. We dropped the prior architectural
    `out = feat + proj_out(U U^T feat)` residual — it had no paper analogue
    and added optimisation complexity for unclear benefit.

Adaptation to depth estimation:

  The paper's main objective (ObjNCELoss, contrastive over prototypes) is
  not directly transferable to a supervised dense-regression task. Instead
  we use M_eicue as the target for two losses, both shared via
  ``scripts.losses``:

    - L_eig (paper Eq. 1): trains the cluster centers C so the assignment
      becomes confident / non-degenerate. Identical to the paper.
    - L_within_cluster: depth-side adaptation of the paper's "within-object
      semantic consistency" — we penalize the soft-assignment-weighted
      variance of predicted depth within each cluster.
"""
