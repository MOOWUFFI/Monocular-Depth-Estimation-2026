#!/usr/bin/env python3
"""Standalone inference for a trained MobileViT-XX-Small + EAGLE depth model.
...
Usage (from the repository root):
    python -m approaches.Mobilevit.inference.inference \
        --ckpt approaches/Mobilevit/results/<run>/best_checkpoint.pt \
        --test_dir /cluster/courses/cil/monocular-depth-estimation/test \
        --out_dir approaches/Mobilevit/results/<run>/predictions \
        --out_csv approaches/Mobilevit/results/<run>/submission.csv
"""
...
from approaches.Mobilevit.model import build_model
...
def preprocess_rgb(rgb_path: str, input_size: int, normalize: bool) -> torch.Tensor:
    ...
    For MobileViT ``normalize=False`` keeps the image in [0, 1]; ImageNet
    normalization is applied only if ``normalize=True``.
    """
