#!/usr/bin/env python3
"""Training entrypoint for the SegFormer-B0 + EAGLE approach.
...
What is trained:
  ...
  Optional:  SegFormer encoder layers (at a reduced LR via --unfreeze_encoder)

Usage (from the repository root):
    # From scratch (encoder loaded from the pretrained SegFormer-B0)
    python -m approaches.Segformer.train.train --from_scratch \
        --eam_stages 2 3 --output_dir approaches/Segformer/results/eagle_eam2s3

    # From a Stage-2 distilled encoder + head
    python -m approaches.Segformer.train.train \
...
from approaches.Segformer.model import DepthDecodeHead, EAGLEDepthModel
from approaches.Segformer.train.dataset import DepthDataset, build_data_splits
from approaches.Segformer.train.losses import EagleDepthLoss
from approaches.Segformer.train.schedulers import build_scheduler
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="EAGLE-augmented fine-tuning of the SegFormer-B0 depth model.",
    ...
    p.add_argument(
        ...
        default=False,
        help=(
            "Train EAGLE directly from the pretrained SegFormer checkpoint "
            "(no Stage-2 distilled encoder/head). When set, --stage2_encoder "
            "and --stage2_head are ignored and the encoder is loaded from "
            "--seg_model_id while the depth head is randomly initialised."
        ),
    )
    p.add_argument(
        "--stage2_encoder",
        ...
        help=(
            ...
            "SegformerModel save). Required unless --from_scratch is set."
        ),
    )
    p.add_argument(
        "--stage2_head",
    ...
    p.add_argument(
        ...
        help="HuggingFace model ID / local path for the SegFormer config + weights.",
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "SegFormer-B0 has stages 0-3: stages 2 (16x16) and 3 (8x8) "
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Unfreeze the SegFormer encoder and train it at encoder_lr_mult x base LR.",
    )
    ...
    p.add_argument("--output_dir",  type=str, default="approaches/Segformer/results")
...
def main() -> None:
    ...
    log.info(
        "Device: %s | AMP: %s | student_type: segformer | EAM stages: %s | unfreeze_encoder: %s",
    ...
    if args.from_scratch:
        run_tag = (
            f"segformer_eagle_scratch_eam{stage_tag}_k{args.eam_k}_c{args.num_clusters}"
            f"_lr{args.lr:.0e}"
        )
    else:
        run_tag = (
            f"eagle_stage3_segformer_eam{stage_tag}_k{args.eam_k}_c{args.num_clusters}"
            f"_lr{args.lr:.0e}"
        )
    ...
    imagenet_norm = True  # SegFormer uses ImageNet normalization
    ...
    if args.from_scratch:
        log.info("from_scratch=True — loading SegFormer encoder from %s …", args.seg_model_id)
        try:
            encoder = SegformerModel.from_pretrained(args.seg_model_id)
        except Exception:
            encoder = SegformerForSemanticSegmentation.from_pretrained(args.seg_model_id).segformer
    else:
        if not args.stage2_encoder:
            raise ValueError("--stage2_encoder is required when --from_scratch is not set.")
        log.info("Loading Stage-2 SegFormer encoder from %s …", args.stage2_encoder)
        ...
        try:
            encoder = SegformerModel.from_pretrained(str(encoder_path))
        except Exception:
            encoder = SegformerForSemanticSegmentation.from_pretrained(str(encoder_path)).segformer
    hidden_sizes: List[int] = list(encoder.config.hidden_sizes)
    log.info("SegFormer hidden_sizes: %s", hidden_sizes)
    ...
    for s in args.eam_stages:
        if s > max_stage:
            raise ValueError(
                f"--eam_stages contains {s} but SegFormer only has stages "
